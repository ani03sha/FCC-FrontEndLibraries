/**
 * So far, you've seen how to use if/else, &&, null and the ternary operator (condition ? expressionIfTrue : expressionIfFalse)
 * to make conditional decisions about what to render and when.
 *
 * However, there's one important topic left to discuss that lets you combine any or all of these
 * concepts with another powerful React feature: props.
 *
 * Using props to conditionally render code is very common with React developers — that is, they use
 * the value of a given prop to automatically make decisions about what to render.
 *
 * In this challenge, you'll set up a child component to make rendering decisions based on props.
 *
 * You'll also use the ternary operator, but you can see how several of the other concepts that were
 ...
 *
 * Statistically speaking, this expression will return true 50% of the time, and false the other 50%.
 *
 * On line 30, replace the comment with this expression to complete the variable declaration.
 *
 ...
 * based on the fiftyFifty prop that's being passed in from GameOfChance.
 *
 * Finally, make sure the handleClick() method is correctly counting each turn so the user knows how many
 * times they've played.
 *
 * This also serves to let the user know the component has actually updated in case they win or lose
 * twice in a row.
 *
 */
